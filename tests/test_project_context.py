...
MODULE = importlib.import_module('agents.venice_private_desk')
...
class ProjectContextTest(unittest.TestCase):
    ...
    def test_primary_names_match(self) -> None:
        spec = MODULE.build_project_spec()
        self.assertEqual(spec.primary_python_module, 'venice_private_desk')
        self.assertEqual(spec.primary_contract_name, 'PrivateDecisionRegistry')
