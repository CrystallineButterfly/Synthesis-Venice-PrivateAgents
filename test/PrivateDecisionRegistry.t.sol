...
contract PrivateDecisionRegistryTest is Test {
    PrivateDecisionRegistry private controller;
    ...
    bytes32 private actionId = keccak256("synthesis_venice_privateagents");
    bytes32 private subjectId = keccak256("subject");

    function setUp() public {
        controller = new PrivateDecisionRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
    ...
    function testExecuteBoundedActionRevertsWithoutReportedBalance() public {
        PrivateDecisionRegistry freshController = new PrivateDecisionRegistry(admin, operator, reporter, 100 ether);
        vm.startPrank(admin);
